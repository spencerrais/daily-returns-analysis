...
Here the library missMDA is used. It does manage to return values, but unable to estimate any values for the number of parameters which should be retained. Here I have just input an arbitrary value, and have done testing with lower values as well which has lead to suspicious holes in the time series plot.
```{r}
library(missMDA)
library(FactoMineR)
...
I also check the plots for the values which have been given by missMDA.
